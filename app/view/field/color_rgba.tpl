...
<input type="range" min="0" max="1" step="0.01" name="field[<?=htmlspecialchars($idParent)?>][<?=$id?>][value][alfa]" value="<?=htmlspecialchars($fieldValue['alfa'])?>" id="field_<?=htmlspecialchars($id.'_2_'.$idParent)?>" class="field colorrgba componentalfa"/>
